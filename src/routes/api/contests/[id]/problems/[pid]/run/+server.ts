...
export const POST = (event) =>
  api(async (event) => {
    ...
    if (now < contest.startsAt || now > contest.endsAt)
      throw new ApiError(403, 'Runs are closed for this contest', 'contest_window_closed');
    ...
    if (!link?.problem) throw new ApiError(404, 'Contest problem not found', 'not_found');
    if (!checkRateLimit(`contest-run:${contest.id}:${user.id}`, CUSTOM_RUN_RATE_LIMIT, CUSTOM_RUN_RATE_WINDOW_MS))
    ...
    try {
      ...
      console.error('API contest custom run failed:', e);
